...
public sealed class TechnicalWriterWorkerTests
{
    ...
    private readonly TechnicalWriterWorker _worker;
    private readonly IServiceScope _scope;

    public TechnicalWriterWorkerTests()
    {
        var scopeFactory = Substitute.For<IServiceScopeFactory>();
        var options = Options.Create(new AgentWorkerOptions { PollingIntervalMs = 1000 });
        var logger = NullLogger<TechnicalWriterWorker>.Instance;

        _worker = new TechnicalWriterWorker(scopeFactory, options, logger);
        ...
        provider.GetService(typeof(ILogger<TechnicalWriterWorker>)).Returns(logger);
    ...
    public async Task ExecuteWorkAsync_HappyPath_ExecutesCompleteCommandSequence()
    {
        ...
        // Assert — attempt created with Documentation type
        await _mediator.Received(1).Send(
            Arg.Is<CreateTaskAttemptCommand>(c =>
                c.WorkTaskId == task.Id &&
                c.Type == AttemptType.Documentation),
    ...
    public async Task ExecuteWorkAsync_WhenLlmThrows_ReleasesAgentAndLeavesTaskInCurrentStatus()
    {
        ...
        // Assert — task NOT completed
        await _mediator.DidNotReceive().Send(
            Arg.Any<CompleteWorkTaskCommand>(),
            Arg.Any<CancellationToken>());
    }
}
