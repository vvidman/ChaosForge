...
public sealed class WorkTaskStatusChangedSignalRHandlerTests
{
    ...
    public WorkTaskStatusChangedSignalRHandlerTests()
    {
        _hubContext.Clients.Returns(_hubClients);
        _hubClients.All.Returns(_clientProxy);
    }

    private WorkTaskStatusChangedSignalRHandler CreateHandler() =>
        new(_hubContext, NullLogger<WorkTaskStatusChangedSignalRHandler>.Instance);
    ...
    public async Task Handle_Always_SendsReceiveEventWithCorrectTypeString()
    {
        ...
        await _clientProxy.Received(1).SendCoreAsync(
            ...
            Arg.Is<object[]>(args =>
                args.Length == 1 &&
                ((SignalRMessage)args[0]).Type == "WorkTaskStatusChanged"),
