...
public sealed class RevisionGateResolvedSignalRHandlerTests
{
    ...
    public RevisionGateResolvedSignalRHandlerTests()
    {
        _hubContext.Clients.Returns(_hubClients);
        _hubClients.All.Returns(_clientProxy);
    }

    private RevisionGateResolvedSignalRHandler CreateHandler() =>
        new(_hubContext, NullLogger<RevisionGateResolvedSignalRHandler>.Instance);

    [Fact]
    public async Task Handle_Always_SendsReceiveEventWithCorrectTypeString()
    {
        // Arrange
        var notification = new RevisionGateResolvedEvent(
            Guid.NewGuid(),
            Guid.NewGuid(),
            RevisionGateAction.Accept);
        ...
        await _clientProxy.Received(1).SendCoreAsync(
            ...
            Arg.Is<object[]>(args =>
                args.Length == 1 &&
                ((SignalRMessage)args[0]).Type == "RevisionGateResolved"),
