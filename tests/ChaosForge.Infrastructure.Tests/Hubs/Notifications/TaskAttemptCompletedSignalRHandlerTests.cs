...
public sealed class TaskAttemptCompletedSignalRHandlerTests
{
    ...
    public TaskAttemptCompletedSignalRHandlerTests()
    {
        _hubContext.Clients.Returns(_hubClients);
        _hubClients.All.Returns(_clientProxy);
    }

    private TaskAttemptCompletedSignalRHandler CreateHandler() =>
        new(_hubContext, NullLogger<TaskAttemptCompletedSignalRHandler>.Instance);

    [Fact]
    public async Task Handle_Always_SendsReceiveEventWithCorrectTypeString()
    {
        // Arrange
        var notification = new TaskAttemptCompletedEvent(
            Guid.NewGuid(),
            Guid.NewGuid(),
        ...
        await _clientProxy.Received(1).SendCoreAsync(
            ...
            Arg.Is<object[]>(args =>
                args.Length == 1 &&
                ((SignalRMessage)args[0]).Type == "TaskAttemptCompleted"),
