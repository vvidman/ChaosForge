...
public sealed class AgentStatusChangedSignalRHandlerTests
{
    ...
    public AgentStatusChangedSignalRHandlerTests()
    {
        _hubContext.Clients.Returns(_hubClients);
        _hubClients.All.Returns(_clientProxy);
    }

    private AgentStatusChangedSignalRHandler CreateHandler() =>
        new(_hubContext, NullLogger<AgentStatusChangedSignalRHandler>.Instance);

    [Fact]
    public async Task Handle_Always_SendsReceiveEventWithCorrectTypeString()
    {
        // Arrange
        var notification = new AgentInstanceStatusChangedEvent(
            Guid.NewGuid(),
            AgentInstanceStatus.Idle,
            AgentInstanceStatus.Working);
        ...
        await _clientProxy.Received(1).SendCoreAsync(
            ...
            Arg.Is<object[]>(args =>
                args.Length == 1 &&
                ((SignalRMessage)args[0]).Type == "AgentStatusChanged"),
