...
public sealed class TaskAttemptResolvedSignalRHandlerTests
{
    ...
    public TaskAttemptResolvedSignalRHandlerTests()
    {
        _hubContext.Clients.Returns(_hubClients);
        _hubClients.All.Returns(_clientProxy);
    }

    private TaskAttemptResolvedSignalRHandler CreateHandler() =>
        new(_hubContext, NullLogger<TaskAttemptResolvedSignalRHandler>.Instance);

    [Fact]
    public async Task Handle_Always_SendsReceiveEventWithCorrectTypeString()
    {
        // Arrange
        var notification = new TaskAttemptResolvedEvent(
            Guid.NewGuid(),
            Guid.NewGuid(),
        ...
        await _clientProxy.Received(1).SendCoreAsync(
            ...
            Arg.Is<object[]>(args =>
                args.Length == 1 &&
                ((SignalRMessage)args[0]).Type == "TaskAttemptResolved"),
