...
public sealed class StartAgentWorkCommandHandlerTests
{
    ...
    public async Task Handle_WhenAgentIsIdle_StartsWorkAndSavesChanges()
    {
        ...
        agentInstance.Status.Should().Be(AgentInstanceStatus.Working);
        agentInstance.CurrentTaskId.Should().Be(taskId);
    ...
    public async Task Handle_WhenAgentNotFound_ReturnsFailure()
    {
        ...
        var command = new StartAgentWorkCommand(agentInstanceId, Guid.NewGuid());
    ...
    [Fact]
    public async Task Handle_WhenAgentAlreadyWorking_ReturnsDomainExceptionMessage()
    {
        // Arrange
        var agentInstance = new AgentInstance(Guid.NewGuid(), AgentRole.Developer, "DevBot-1");
        agentInstance.StartWork(Guid.NewGuid());
        var command = new StartAgentWorkCommand(agentInstance.Id, Guid.NewGuid());
        _agentInstanceRepository.GetByIdAsync(agentInstance.Id, Arg.Any<CancellationToken>()).Returns(agentInstance);
        var handler = CreateHandler();
