...
public sealed class StartWorkTaskCommandHandlerTests
{
    ...
    public async Task Handle_WhenTaskNotFound_ReturnsFailure()
    {
        ...
        var command = new StartWorkTaskCommand(taskId);
    ...
    [Fact]
    public async Task Handle_WhenNoSprintAssigned_ReturnsDomainExceptionMessage()
    {
        // Arrange
        var task = new WorkTask(Guid.NewGuid(), "Title", "Description", 2);
        var command = new StartWorkTaskCommand(task.Id);
        ...
        // Assert
        result.IsSuccess.Should().BeFalse();
        result.Error.Should().Contain("no sprint");
        await _unitOfWork.DidNotReceive().SaveChangesAsync(Arg.Any<CancellationToken>());
    }
}
