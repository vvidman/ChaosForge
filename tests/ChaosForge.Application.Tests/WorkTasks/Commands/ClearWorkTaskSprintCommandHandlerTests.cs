...
public sealed class ClearWorkTaskSprintCommandHandlerTests
{
    private readonly IWorkTaskRepository _workTaskRepository = Substitute.For<IWorkTaskRepository>();
    private readonly IUnitOfWork _unitOfWork = Substitute.For<IUnitOfWork>();

    private ClearWorkTaskSprintCommandHandler CreateHandler() =>
        new(_workTaskRepository, _unitOfWork);

    [Fact]
    public async Task Handle_WhenTaskExists_ClearsSprintIdAndSavesChanges()
    {
        // Arrange
        var task = new WorkTask(Guid.NewGuid(), "Title", "Description", 2);
        task.AssignToSprint(Guid.NewGuid());
        var command = new ClearWorkTaskSprintCommand(task.Id);
    ...
    public async Task Handle_WhenTaskNotFound_ReturnsFailure()
    {
        ...
        var command = new ClearWorkTaskSprintCommand(taskId);
