...
public sealed class AssignWorkTaskToSprintCommandHandlerTests
{
    private readonly IWorkTaskRepository _workTaskRepository = Substitute.For<IWorkTaskRepository>();
    private readonly IUnitOfWork _unitOfWork = Substitute.For<IUnitOfWork>();

    private AssignWorkTaskToSprintCommandHandler CreateHandler() =>
        new(_workTaskRepository, _unitOfWork);

    [Fact]
    public async Task Handle_WhenTaskExists_AssignsSprintAndSavesChanges()
    {
        // Arrange
        var task = new WorkTask(Guid.NewGuid(), "Title", "Description", 2);
        var sprintId = Guid.NewGuid();
        var command = new AssignWorkTaskToSprintCommand(task.Id, sprintId);
    ...
    public async Task Handle_WhenTaskNotFound_ReturnsFailure()
    {
        ...
        var command = new AssignWorkTaskToSprintCommand(taskId, Guid.NewGuid());
    ...
    [Fact]
    public async Task Handle_WhenTaskNotInBacklog_ReturnsDomainExceptionMessage()
    {
        // Arrange
        var task = new WorkTask(Guid.NewGuid(), "Title", "Description", 2);
        task.AssignToSprint(Guid.NewGuid());
        task.Start();
        var command = new AssignWorkTaskToSprintCommand(task.Id, Guid.NewGuid());
        _workTaskRepository.GetByIdAsync(task.Id, Arg.Any<CancellationToken>()).Returns(task);
        var handler = CreateHandler();
