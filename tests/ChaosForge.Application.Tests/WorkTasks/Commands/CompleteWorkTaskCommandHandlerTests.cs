...
public sealed class CompleteWorkTaskCommandHandlerTests
{
    private readonly IWorkTaskRepository _workTaskRepository = Substitute.For<IWorkTaskRepository>();
    private readonly IUnitOfWork _unitOfWork = Substitute.For<IUnitOfWork>();

    private CompleteWorkTaskCommandHandler CreateHandler() =>
        new(_workTaskRepository, _unitOfWork);

    [Fact]
    public async Task Handle_WhenTaskIsInDocumentation_CompletesAndSavesChanges()
    {
        // Arrange
        ...
        task.Approve();
        task.PassTesting();
        var command = new CompleteWorkTaskCommand(task.Id);
    ...
    public async Task Handle_WhenTaskNotFound_ReturnsFailure()
    {
        ...
        var command = new CompleteWorkTaskCommand(taskId);
    ...
    [Fact]
    public async Task Handle_WhenTaskNotInDocumentation_ReturnsDomainExceptionMessage()
    {
        // Arrange
        var task = new WorkTask(Guid.NewGuid(), "Title", "Description", 2);
        var command = new CompleteWorkTaskCommand(task.Id);
