...
public sealed class RejectWorkTaskCommandHandlerTests
{
    private readonly IWorkTaskRepository _workTaskRepository = Substitute.For<IWorkTaskRepository>();
    private readonly IUnitOfWork _unitOfWork = Substitute.For<IUnitOfWork>();

    private RejectWorkTaskCommandHandler CreateHandler() =>
        new(_workTaskRepository, _unitOfWork);

    [Fact]
    public async Task Handle_WhenTaskIsInReview_RejectsAndSavesChanges()
    {
        // Arrange
        ...
        task.Start();
        task.SendToReview();
        var command = new RejectWorkTaskCommand(task.Id);
    ...
    public async Task Handle_WhenTaskNotFound_ReturnsFailure()
    {
        ...
        var command = new RejectWorkTaskCommand(taskId);
    ...
    [Fact]
    public async Task Handle_WhenTaskNotRejectable_ReturnsDomainExceptionMessage()
    {
        // Arrange
        var task = new WorkTask(Guid.NewGuid(), "Title", "Description", 2);
        var command = new RejectWorkTaskCommand(task.Id);
