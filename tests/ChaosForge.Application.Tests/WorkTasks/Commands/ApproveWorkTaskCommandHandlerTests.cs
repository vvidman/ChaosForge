...
public sealed class ApproveWorkTaskCommandHandlerTests
{
    private readonly IWorkTaskRepository _workTaskRepository = Substitute.For<IWorkTaskRepository>();
    private readonly IUnitOfWork _unitOfWork = Substitute.For<IUnitOfWork>();

    private ApproveWorkTaskCommandHandler CreateHandler() =>
        new(_workTaskRepository, _unitOfWork);

    [Fact]
    public async Task Handle_WhenTaskIsInReview_ApprovesAndSavesChanges()
    {
        // Arrange
        ...
        task.Start();
        task.SendToReview();
        var command = new ApproveWorkTaskCommand(task.Id);
        ...
        task.Status.Should().Be(WorkTaskStatus.InTesting);
    ...
    public async Task Handle_WhenTaskNotFound_ReturnsFailure()
    {
        ...
        var command = new ApproveWorkTaskCommand(taskId);
    ...
    [Fact]
    public async Task Handle_WhenTaskNotInReview_ReturnsDomainExceptionMessage()
    {
        // Arrange
        var task = new WorkTask(Guid.NewGuid(), "Title", "Description", 2);
        var command = new ApproveWorkTaskCommand(task.Id);
