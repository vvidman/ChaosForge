...
public sealed class SendWorkTaskToReviewCommandHandlerTests
{
    private readonly IWorkTaskRepository _workTaskRepository = Substitute.For<IWorkTaskRepository>();
    private readonly IUnitOfWork _unitOfWork = Substitute.For<IUnitOfWork>();

    private SendWorkTaskToReviewCommandHandler CreateHandler() =>
        new(_workTaskRepository, _unitOfWork);

    [Fact]
    public async Task Handle_WhenTaskIsInProgress_SendsToReviewAndSavesChanges()
    {
        // Arrange
        var task = new WorkTask(Guid.NewGuid(), "Title", "Description", 2);
        task.AssignToSprint(Guid.NewGuid());
        task.Start();
        var command = new SendWorkTaskToReviewCommand(task.Id);
    ...
    public async Task Handle_WhenTaskNotFound_ReturnsFailure()
    {
        ...
        var command = new SendWorkTaskToReviewCommand(taskId);
    ...
    [Fact]
    public async Task Handle_WhenTaskNotInProgress_ReturnsDomainExceptionMessage()
    {
        // Arrange
        var task = new WorkTask(Guid.NewGuid(), "Title", "Description", 2);
        var command = new SendWorkTaskToReviewCommand(task.Id);
