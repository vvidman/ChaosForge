...
public sealed class DeleteWorkTaskCommandHandlerTests
{
    private readonly IWorkTaskRepository _workTaskRepository = Substitute.For<IWorkTaskRepository>();
    private readonly IUnitOfWork _unitOfWork = Substitute.For<IUnitOfWork>();

    private DeleteWorkTaskCommandHandler CreateHandler() =>
        new(_workTaskRepository, _unitOfWork);

    [Fact]
    public async Task Handle_WhenTaskExists_DeletesTaskAndSavesChanges()
    {
        // Arrange
        var task = new WorkTask(Guid.NewGuid(), "Title", "Description", 2);
        var command = new DeleteWorkTaskCommand(task.Id);
        ...
        _workTaskRepository.Received(1).Delete(task);
    ...
    public async Task Handle_WhenTaskNotFound_ReturnsFailure()
    {
        ...
        var command = new DeleteWorkTaskCommand(taskId);
        ...
        _workTaskRepository.DidNotReceive().Delete(Arg.Any<WorkTask>());
