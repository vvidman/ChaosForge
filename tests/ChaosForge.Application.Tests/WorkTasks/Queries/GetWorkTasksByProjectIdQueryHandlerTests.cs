...
public sealed class GetWorkTasksByProjectIdQueryHandlerTests
{
    private readonly IWorkTaskRepository _workTaskRepository = Substitute.For<IWorkTaskRepository>();

    private GetWorkTasksByProjectIdQueryHandler CreateHandler() =>
    ...
    public async Task Handle_WhenNoTasksExist_ReturnsSuccessWithEmptyList()
    {
        ...
        _workTaskRepository.GetByProjectIdAsync(projectId, Arg.Any<CancellationToken>())
            .Returns(new List<WorkTask>().AsReadOnly());

        var handler = CreateHandler();

        // Act
        var result = await handler.Handle(new GetWorkTasksByProjectIdQuery(projectId), CancellationToken.None);
