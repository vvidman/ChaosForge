...
internal sealed class WorkTaskStatusChangedSignalRHandler
    : INotificationHandler<WorkTaskStatusChangedEvent>
{
    private readonly IHubContext<ChaosForgeHub> _hubContext;
    private readonly ILogger<WorkTaskStatusChangedSignalRHandler> _logger;

    public WorkTaskStatusChangedSignalRHandler(
        IHubContext<ChaosForgeHub> hubContext,
        ILogger<WorkTaskStatusChangedSignalRHandler> logger)
    {
    ...
    public async Task Handle(WorkTaskStatusChangedEvent notification, CancellationToken cancellationToken)
    {
        try
        {
            var message = new SignalRMessage("WorkTaskStatusChanged", new
            {
        ...
        {
            _logger.LogError(ex, "SignalR broadcast failed for {Event}", nameof(WorkTaskStatusChangedEvent));
        }
    }
}
