...
public record GetWorkTasksByProjectIdQuery(Guid ProjectId) : IRequest<Result<IReadOnlyList<WorkTaskDto>>>;

internal sealed class GetWorkTasksByProjectIdQueryHandler(IWorkTaskRepository workTaskRepository)
    : IRequestHandler<GetWorkTasksByProjectIdQuery, Result<IReadOnlyList<WorkTaskDto>>>
{
    public async Task<Result<IReadOnlyList<WorkTaskDto>>> Handle(
        GetWorkTasksByProjectIdQuery request,
        CancellationToken cancellationToken)
    {
        var tasks = await workTaskRepository.GetByProjectIdAsync(request.ProjectId, cancellationToken);
