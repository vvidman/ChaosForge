...
public record BlockAgentCommand(Guid AgentInstanceId) : IRequest<Result>;

public sealed class BlockAgentCommandValidator : AbstractValidator<BlockAgentCommand>
{
    public BlockAgentCommandValidator()
    {
        RuleFor(x => x.AgentInstanceId).NotEmpty().WithMessage("AgentInstanceId must not be empty.");
    }
}

internal sealed class BlockAgentCommandHandler(
    IAgentInstanceRepository agentInstanceRepository,
    IUnitOfWork unitOfWork)
    : IRequestHandler<BlockAgentCommand, Result>
{
    public async Task<Result> Handle(BlockAgentCommand request, CancellationToken cancellationToken)
    {
